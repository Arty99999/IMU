...
{
    static uint8_t TAddr = 0;

	 MPU6050_t* mpu6050  =(MPU6050_t*)mpu6050_data ;
	
    MPU6050_WriteByte(0x6B, 0x01,mpu6050->i2cHandler); // 休眠
    DWT_Delay_s(0.5);
    ...
    MPU6050_WriteByte(0x6B, 0x00,mpu6050->i2cHandler); // 解除休眠状态0x00
    DWT_Delay_s(0.05);
    MPU6050_WriteByte(0x19, 0x00,mpu6050->i2cHandler); // 采样频率（1KHz）
    DWT_Delay_s(0.05);
    MPU6050_WriteByte(0x1A, 0x03,mpu6050->i2cHandler); // 低通滤波
    DWT_Delay_s(0.05);
    MPU6050_WriteByte(0x1B, 0x10,mpu6050->i2cHandler); // 陀螺仪量程，当寄存器0x1B的值为0x10时，陀螺仪量程为
    DWT_Delay_s(0.05);
    MPU6050_WriteByte(0x1C, 0x09,mpu6050->i2cHandler); // 加速度量程，当寄存器0x1C的值为0x09时，加速度量程为
    DWT_Delay_s(0.05);
